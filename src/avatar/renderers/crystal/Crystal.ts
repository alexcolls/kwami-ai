...
import { defaultCrystalConfig } from './config'
...
/**
 * Crystal - A geometric 3D avatar made of floating crystal shards
...
export class Crystal {
  ...
  private core: Mesh
  private glow: Mesh
  private config = defaultCrystalConfig

  public currentFormation: CrystalFormationSelection
  private materials: {
  ...
  // Audio effects
  public audioEffects: CrystalAudioEffects
  ...
  constructor(private options: CrystalOptions) {
  ...
  setFormation(selection: CrystalFormationSelection): void {
    ...
    logger.info(`Crystal formation changed to: ${selection.formation}`)
  }

  /**
   * Get current formation
   */
  getFormation(): CrystalFormationSelection {
  ...
  setColors(primary: string, secondary: string, accent: string): void {
    ...
    logger.debug(`Crystal colors updated`)
  }
  ...
  setScale(scale: number): void {
    ...
    this.group.scale.setScalar(this.scale)
    logger.debug(`Crystal scale set to: ${this.scale}`)
  }
  ...
  async startListening(): Promise<void> {
    try {
      ...
      logger.info('🎤 Crystal started listening')
    } catch (error) {
  ...
  stopListening(): void {
    ...
    logger.info('🔇 Crystal stopped listening')
  }
  ...
  startThinking(): void {
    ...
    logger.info(`🤔 Crystal started thinking (${this.thinkingDuration / 1000}s)`)
  ...
  stopThinking(): void {
    ...
    logger.info('💭 Crystal stopped thinking')
  }
  ...
  /**
   * Randomize crystal appearance
   */
  setRandomCrystal(): void {
    // Random formation
